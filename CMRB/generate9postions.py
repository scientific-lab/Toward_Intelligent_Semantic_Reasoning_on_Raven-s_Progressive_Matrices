...
Name=os.listdir('D:\\zsk\\SRAN-master\\I-RAVEN\\distribute_four')
k=0
for name_ind in range (0,10000):
    ...
    for n in Name:
        if n[0:length]=='RAVEN_'+str(name_ind)+'_' and n[-3:]=='npz':
            ...
            A=np.load('D:\\zsk\\SRAN-master\\I-RAVEN\\distribute_four\\'+n)
            data=A['image']
            answer=A['target']
            ...
            Exist1=Exist[:,:,8:16]
            np.save('D:\\zsk\\RAVEN\\position9\\'+str(k)+'.npy',Exist0)
            np.save('D:\\zsk\\RAVEN\\pselection9\\'+str(k)+'.npy',Exist1)
            k+=1
