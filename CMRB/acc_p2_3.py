import numpy as np
from relation_position_3 import relation_map,two_ship,three_ship,solve_r2_return,solve_r3_return
def cal_al(relationship_two_30,relationship_three_30,L0,L1):
    ...
    for ii in range (0,10000):
        ...
        c=np.load('D:\\zsk\\RAVEN\\pselection9_3\\'+str(ii)+'.npy')
    ...
    for a in range (0,30):
        for b in range (0,30):
            ...
            if set(l1[a])<set(l[b]):
                if a not in ll2:
                    ll2.append(a)
    np.save('l_position_no_3'+str(L0)+'_'+str(L1)+'.npy',l)
    np.save('l1_position_no_3'+str(L0)+'_'+str(L1)+'.npy',l1)
