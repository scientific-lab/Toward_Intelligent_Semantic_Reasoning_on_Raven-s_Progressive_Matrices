...
Name=os.listdir('D:\\zsk\\SRAN-master\\I-RAVEN-test\\distribute_nine')
k=0
for name_ind in range (0,5000):
    ...
    for n in Name:
        if n[0:length]=='RAVEN_'+str(name_ind)+'_' and n[-3:]=='npz':
            ...
            A=np.load('D:\\zsk\\SRAN-master\\I-RAVEN-test\\distribute_nine\\'+n)
            data=A['image']
            answer=A['target']
            ...
            Exist1=Exist[:,:,8:16]
            np.save('D:\\zsk\\RAVEN\\position9_3_test\\'+str(k)+'.npy',Exist0)
            np.save('D:\\zsk\\RAVEN\\pselection9_3_test\\'+str(k)+'.npy',Exist1)
            k+=1
