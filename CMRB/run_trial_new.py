...
for L0 in [4]:
    for L1 in [2]:
        ...
        for ii in range (0,30000):
            s=0
            s1=1
            d=np.load('D:\\zsk\\RAVEN\\data_9_1\\'+str(ii)+'.npy')
...
#                 np.save('CM5\\ts3_final'+str(ii)+'_'+str(L0)+'_'+str(L1)+'.npy',relationship_three_30)
                ...
                np.save('CM7\\ts2_final'+'_'+str(L0)+'_'+str(L1)+'-'+str(ii)+'.npy',relationship_two_30)
                np.save('CM7\\ts3_final'+'_'+str(L0)+'_'+str(L1)+'-'+str(ii)+'.npy',relationship_three_30)
                #o,p=cal_al(relationship_two_30,relationship_three_30,L0,L1,ii)
                #print(o)
                #print(p)
                np.save('CM7\\o_final'+str(L0)+'_'+str(L1)+'.npy',o)
                np.save('CM7\\p_final'+str(L0)+'_'+str(L1)+'.npy',p)
            if acc_t>0.997 and o.size==23 and p.size==24:
            ...
            if ii==29999:
                relationship_two_30[:,:,np.where(count<2)[0]]=0
                relationship_three_30[:,:,:,np.where(count1<2)[0]]=0
        np.save('CM7\\ts2_final'+'_'+str(L0)+'_'+str(L1)+'.npy',relationship_two_30)
        np.save('CM7\\ts3_final'+'_'+str(L0)+'_'+str(L1)+'.npy',relationship_three_30)
        o,p=cal_al(relationship_two_30,relationship_three_30,L0,L1,ii)
        #print(o)
        #print(p)
        np.save('CM7\\o_final'+str(L0)+'_'+str(L1)+'.npy',o)
...
# print(acc_all)
np.save('CM7\\acc_final.npy',acc_all)
