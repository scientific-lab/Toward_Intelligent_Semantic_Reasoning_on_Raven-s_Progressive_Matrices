...
def cal_al(relationship_two_30,relationship_three_30,L0,L1,tt):
    s=0
    l=[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[]]
    l1=[[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[],[]]
    ll1=[]
    ...
    for ii in range (0,tt):
        ...
        c=np.load('D:\\zsk\\RAVEN\\select8\\'+str(ii)+'.npy')
    ...
    for a in range (0,30):
        for b in range (0,30):
            ...
            if set(l1[a])<set(l[b]):
                if a not in ll2:
                    ll2.append(a)
    np.save('l_no3'+str(L0)+'_'+str(L1)+'.npy',l)
    np.save('l1_no3'+str(L0)+'_'+str(L1)+'.npy',l1)
