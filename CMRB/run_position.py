import numpy as np
from relation_position import relation_map,two_ship,three_ship,solve_r2,solve_r3
from acc_p1 import cal_acc
from acc_p2 import cal_al
acc_all=np.zeros((4,2))
for L0 in range (2,6):
    for L1 in range (1,3):
        ...
        tr=np.zeros(30)
        tr1=np.zeros(30)
        d_old=np.zeros((2,2,9,30))
        d_old1=np.zeros((2,2,9,30))
        for ii in range (0,10000):
                ...
                for a in range (0,eq2_0.size):
                    if relationship_two_30[:,:,i][eq2_0[a]][eq2_1[a]]!=0:
                        if relationship_two[eq2_0[a]][eq2_1[a]]!=0:
                            L1_2+=1
                if L1_2>=L0:
                    if relationship_two_30[:,:,i][relationship_two_30[:,:,i]>0].size>6:
                        relationship_two_copy=relationship_two.copy()
                        relationship_two_copy[relationship_two_copy!=relationship_two_30[:,:,i]]=0
                        if relationship_two_copy[relationship_two_copy>0].size>=6:
            ...
            if ii%1000==1:
                ...
                print('count1')
                print(count1)
                np.save('CMP_2\\ts2'+str(ii)+'_'+str(L0)+'_'+str(L1)+'.npy',relationship_two_30)
                np.save('CMP_2\\ts3'+str(ii)+'_'+str(L0)+'_'+str(L1)+'.npy',relationship_three_30)
            if ii==9999:
                relationship_two_30[:,:,np.where(count<2)[0]]=0
                relationship_three_30[:,:,:,np.where(count1<2)[0]]=0
        np.save('CMP_2\\ts2'+'_'+str(L0)+'_'+str(L1)+'.npy',relationship_two_30)
        np.save('CMP_2\\ts3'+'_'+str(L0)+'_'+str(L1)+'.npy',relationship_three_30)
        o,p=cal_al(relationship_two_30,relationship_three_30,L0,L1)
        print(o)
        print(p)
        np.save('CMP_2\\o'+str(L0)+'_'+str(L1)+'.npy',o)
        np.save('CMP_2\\p'+str(L0)+'_'+str(L1)+'.npy',p)
        acc_all[L0-2,L1-1]=cal_acc(relationship_two_30,relationship_three_30,L0,L1,o,p)
        print(acc_all[L0-2,L1-1])
print(acc_all)
np.save('CMP_2\\acc.npy',acc_all)
